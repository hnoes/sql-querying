-- Comments in SQL Start with dash-dash --

--Question 1: Find the app with an ID of 1880.--
SELECT * FROM apps WHERE ID = 1880; 




-- Question 2: Find the ID and app name for all apps that were last updated on August 01, 2018.
SELECT ID, app_name
FROM apps
WHERE date(last_updated)='2018-08-01';




-- Question 3: Count the number of apps in each category, e.g. “Family | 1972”.
SELECT CONCAT(category, ' | ', COUNT(*)) AS category_count
FROM apps
GROUP BY category; 



--Question 4: Find the top 5 most-reviewed apps and the number of reviews for each.
SELECT app_name, reviews_count
FROM apps
ORDER BY reviews_count DESC 
LIMIT 5;




--Question 5: Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT app_name, reviews_count
FROM apps
WHERE rating >= 4.8
ORDER BY reviews_count DESC 
LIMIT 1;





--Question 6: Find the average rating for each category ordered by the highest rated to lowest rated.
SELECT category, AVG(rating) AS avg_rating
FROM apps
GROUP BY category
ORDER BY avg_rating DESC; 





--Question 7: Find the name, price, and rating of the most expensive app with a rating that’s less than 3.
SELECT app_name, price, rating 
FROM apps
WHERE rating < 3
ORDER BY price DESC 
LIMIT 1;  





--Question 8: Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.
SELECT app_name, min_installs, rating 
FROM apps
WHERE min_installs <= 50 AND rating IS NOT NULL
ORDER BY rating DESC; 





--Question 9: Find the names of all apps that are rated less than 3 with at least 10000 reviews.
SELECT app_name
FROM apps
WHERE rating < 3 AND reviews_count >= 10000;





--Question 10: Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
SELECT app_name, price, reviews_count
FROM apps
WHERE price >= 0.10 AND price <= 1.00
ORDER BY reviews_count DESC
LIMIT 10; 





--Question 11: Find the most out of date app. Hint: You don’t need to do it this way, but it’s possible to do with a subquery: http://www.postgresqltutorial.com/postgresql-max-function/
SELECT app_name, last_updated
FROM apps
WHERE last_updated = (SELECT MAX(last_updated) FROM apps);




--Question 12: Find the most expensive app (the query is very similar to #11).
SELECT app_name, price
FROM apps
WHERE price = (SELECT MAX(price) FROM apps);





--Question 13: Count all the reviews in the Google Play Store.
SELECT SUM(reviews_count) AS total_reviews  
FROM apps;





--Question 14: Find all the categories that have more than 300 apps in them.
SELECT category
FROM apps
GROUP BY category
HAVING COUNT(*) > 300; 





--Question 15: Find the app that has the highest proportion of min_installs to reviews, among apps that have been installed at least 100,000 times. Display the name of the app along with the number of reviews, the min_installs, and the proportion.
SELECT app_name, reviews_count, min_installs,
        CAST(min_installs AS FLOAT) / reviews_count AS proportion
FROM apps
WHERE min_installs >= 100000
ORDER BY proportion DESC
LIMIT 1; 